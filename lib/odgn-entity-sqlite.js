...
_.extend(SqliteStorage.prototype, {
    ...
    registerComponent: function( componentDef, options, callback ){
        ...
        self._open(function(err,db){
            ...
            log.debug('insert component ' + query.sql );
            print_ins( query ); //odgnEntitySchema.getProperties(componentDef.schema) );
            
            var values = [ componentDef.schema.id, JSON.stringify(componentDef.schema), ]

            log.error("FINISH OFF THE SETTING OF QUERY PARAMETERS USING THE VALUES OBJECT");
            process.exit();
            db.run(query.sql, [0], function(err){
            //     if( err ){
            //         return callback(err);
            //     }
