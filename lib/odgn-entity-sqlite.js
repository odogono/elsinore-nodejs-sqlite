...
_.extend(SqliteStorage.prototype, Backbone.Events, {
    ...
    getEntity: function(entity, options, callback){
        ...
        return this.getRow( "SELECT * FROM tbl_entity WHERE id = ?", entity.id, function(err,row){
            ...
            // if( options.debug ){ log.debug('row: ' + JSON.stringify(row)); }
            if( !row ){
                return callback();
            }
            // if( options.debug ){ print_ins( resp ); process.exit(); log.debug('entity ' + JSON.stringify(entity) ) };
    ...
    updateComponent: function( components, options, callback ){
        ...
        var query = Schema.toUpdate( [schemaId, '/schema/component_defaults'], columnNames, {debug:false} );
        ...
        this.dbHandle.serialize( function(){
    ...
    retrieveComponent: function( componentDefOrSchemaId, options, callback ){
        var self = this, componentDef = this._toComponentDef(componentDefOrSchemaId);
        var stmt = Schema.toSelect( componentDef.schema.id, _.extend( {},options, {includeEntity:true} ) );

        // if( options.debug ){ log.debug('retrieveComponent: ' + stmt.getSql()); }
    ...
    getEntityComponents: function( entity, options, callback ){
        ...
        var processEntity = function(err,entity){
             // if( options.debug ){ print_ins( arguments ); process.exit(); log.debug('getEntityComponents: ' + JSON.stringify(entity) );}
             ...
             if( componentDefIds.length <= 0 ){
                 if( options.debug ) log.debug('getEntityComponents: no components found for entity ' + entity.id );
                 return callback( null, [], entity );
             }
    ...
    getComponentForEntity:function( componentDef, entity, callback ){
        ...
        var query = Schema.toSelect( componentDef.schema.id, {where:{entity_id:entity.id}} );
        
        // log.debug('ms getComponentForEntity ' + componentDef.id + ' ' + entity.id + ' ' + query.getSql() + ' ' + JSON.stringify(query.getParameters()) );
        ...
        return self.allRows( query.getSql(), query.getParameters(), function(err, rows){
            ...
            // log.debug('returned ' + rows.length + ' rows');
