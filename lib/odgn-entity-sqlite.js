...
_.extend(SqliteStorage.prototype, Backbone.Events, {
    ...
    updateComponent: function( components, entity, options, callback ){
        ...
        return this.dbHandle.serialize( function(){
            ...
            return async.mapSeries( components,
                function(component,componentCallback){
                    if( !component.id ){
    ...
    getEntityComponents: function( entity, options, callback ){
        ...
        var processEntity = function(err,entity){
             if( options.debug ){ print_ins( arguments ); log.debug('getEntityComponents: ' + JSON.stringify(entity) );}
            ...
            return async.eachSeries( componentDefIds, 
                function(cDef,cDefIdCb){
                    return self.getComponentForEntity( cDef, entity, function(err,components){
                        var componentName;
                        if( options.decorate ){
                        ...
                        if( options.decorate ){
                            entity[ componentName ] = components;
                        }
                        return cDefIdCb(err);
                    });
                },
                function(err){
                    if( err ){ return callback(err); }
                    return callback( null, resultComponentArray, entity );
                });
        };
    ...
    */
    ...
    execQuery: function( query, options, callback ){
        ...
        if( options.debug ){
            // log.debug("HHAHAHA " + JSON.stringify(query.getValues(options.debug)) );
            // log.debug( query.getSql() + ' ' + JSON.stringify(values) );
